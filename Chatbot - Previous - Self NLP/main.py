...
positive_responses = ["yes", "yash", "y", "yeah", "yep",
                      "yea", "ya", "yup", "haa", "ha", "hai", "got it", "i have"]

extra_positive_responses = [
    "i have", "got it", "i am having", "yes", "maybe i am having", "maybe i have", "sometimes", "kind of"]

disease_dict = {
    "diarrhea": {
        "symptom_terms": ["1abdomen pain", "1abdominal pain", "1pain in my abdomen", "1abdomen is paining", "2stomach ache", "2stomach pain", "2stomach is paining", "2pain in my stomach", "3nausea", "3uncomfortable", "5vomit", "5vomiting", "6fever", "6feverish", "6feeling cold", "6having cold", "7bloody & watery loose stools", "7bloody stools", "7watery stools", "7watery loose stools", "7loose stools", "8dehydration", "8dehydrated", "8dehydrating"],
    ...
    "cold and flu": {
        "symptom_terms": ["1fever", "1feverish", "2headache", "2head pain", "2pain in my head", "2head is paining", "3pain and fatigue", "3severe pain", "3fatigue", "4cough", "4dry cough", "4coughing" "5sinus", "6ear infections", "6infections in my ear", "7sore throat", "7throat is sore", "7throat sore", "8runny nose", "8nose is runny", "8nose is running", "8running nose", "9stuffy nose", "9nose is stuffy", "10sneeze", "10sneezing", "10sneezy"],
    ...
    "Hypertension": {
        "symptom_terms": ["1headache", "1pain in my head", "2increased blood pressure", "2increased b p", "2increased bp", "2high bp", "2high b p", "2high blood pressure", "3breathing problem", "3problems in breathing", "3breathing issues", "4sweating", "4sweat", "5bleeding nose", "5nose is bleeding", "6tiredness", "6tired", "7edema", "7swelling", "8irregular heartbeats", "8irregular heart beats", "9chest pain", "10trouble in sleeping"],
...
def illness_working_main(sentence):
    ...
    for i in range(len(possibilities)):
        if possibilities[i] != '':
            if int(possibilities[i][-1]) > maxi:
                maxi = int(possibilities[i][-1])
                index = i
    index_all = []
    possible_disease = ""
    for i in range(len(possibilities)):
        if possibilities[i] != '':
            if int(possibilities[i][-1]) == maxi:
                possible_disease = possible_disease + extract_disease(
                    possibilities[i]) + ", "
                index_all.append(i)
    possible_disease = possible_disease[0:(len(possible_disease)-2)]
    reply = "You might be having " + possible_disease + "."
    multi_disease = possible_disease.split(',')
    mentioned = []
    for i in range(len(possibilities)):
        if i in index_all:
            mentioned.append(possibilities[i].split(','))
    mentioned_id = []
    mentioned_id_multiple = []
    for k in range(len(mentioned)):
        mentioned_id = []
        for i in range(1, (len(mentioned[k])-1), 1):
            mentioned_id.append(mentioned[k][i][0])
        mentioned_id.append(mentioned[k][0])
        mentioned_id_multiple.append(mentioned_id)
    print("Bot:", end=' ')
    ...
    c = 0
    symptom_similarity_count = []
    answered_ids = []
    for d_id, d_info in disease_dict.items():
        for g in range(len(mentioned_id_multiple)):
            c = 0
            if d_id == mentioned_id_multiple[g][len(mentioned_id_multiple[g])-1]:
                for key in d_info:
                    if key == "symptom_terms":
                        temp = ",".join(map(str, d_info[key]))
                        parts = temp.split(',')
                        for p in parts:
                            if (p[0] not in mentioned_id_multiple[g]) and (p[0] not in answered_ids):
                                print("    ", end=' ')
                        ...
                        symptom_similarity_count.append(c)
            answered_ids = []
    print()
    print("Bot:", end=' ')
    extra_check = 0
    for g in range(len(mentioned_id_multiple)):
        if symptom_similarity_count[g] != 0:
            extra_check = extra_check + 1
            typing_animation(
                f"You have {symptom_similarity_count[g]} more symptoms of {mentioned_id_multiple[g][len(mentioned_id_multiple[g])-1]}.")
            print("    ", end=' ')
    if extra_check == 0 and len(mentioned_id_multiple) > 1:
        typing_animation(
            f"You don't show any other symptoms of {possible_disease}, it could be any of those diseases, we recommend you to consult a doctor regarding your signs of illness.")
        raise SystemExit
    elif extra_check == 0 and len(mentioned_id_multiple) == 1:
        typing_animation(f"You are probably having {possible_disease} itself.")
        print()
        print("Bot:", end=' ')
        ...
        m = check_max_index(symptom_similarity_count)
        typing_animation(
            f"We suspect that you are having {mentioned_id_multiple[m][len(mentioned_id_multiple[m])-1]}.")
        if len(mentioned_id_multiple) > 1:
            typing_animation(
                f"We also realised that you've got some separate symptoms which are not an effect of {mentioned_id_multiple[m][len(mentioned_id_multiple[m])-1]}.")
        print()
        print("Bot:", end=' ')
        avoidance(mentioned_id_multiple[m][len(mentioned_id_multiple[m])-1])
        print()
        print("Bot:", end=' ')
        remedy(mentioned_id_multiple[m][len(mentioned_id_multiple[m])-1])
        print()
        print("Bot:", end=' ')
        cure(mentioned_id_multiple[m][len(mentioned_id_multiple[m])-1])
